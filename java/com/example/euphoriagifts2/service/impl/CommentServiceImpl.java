...
public class CommentServiceImpl implements CommentService {
    ...
    public CommentViewModel createComment(CommentServiceModel serviceModel) {
        UserEntity userByUsername = userService.findByUsername(serviceModel.getCreator());

        if (userByUsername == null) {
            throw new UsernameNotFoundException("User with username " + serviceModel.getCreator() + " does not exist!z");
        }

        GiftEntity giftById = giftService.findGiftById(serviceModel.getGiftId());

        if (giftById == null) {
            throw new ObjectNotFoundException("Gift with id: " + serviceModel.getGiftId() + " was not found!");
        }

        CommentEntity newComment = new CommentEntity();
        newComment.setMessage(serviceModel.getMessage());
