...
public class OrderServiceImpl implements OrderService {
    ...
    public Integer countOrders() {
        LocalDate start = YearMonth.now().minusMonths(1).atDay(1).minusDays(1);
        LocalDate end = YearMonth.now().minusMonths(1).atEndOfMonth().plusDays(1);

        return this.orderRepository.findAll().stream().filter(o -> {
            return o.getOrderTime().toLocalDate().isAfter(start) && o.getOrderTime().toLocalDate().isBefore(end);
