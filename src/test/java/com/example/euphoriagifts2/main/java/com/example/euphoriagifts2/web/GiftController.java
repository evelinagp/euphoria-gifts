...
public class GiftController {
    ...
    public String details(@PathVariable Long id, Model model) {
        ...
        RoleEntity roleEntity = giftById.getUserEntity().getRoles().stream().filter(r -> r.getName().equals(RoleNameEnum.ADMIN)).findFirst().orElse(null);
//        giftService.findRoleByGiftById(id);
        model.addAttribute("isAdmin", roleEntity);
...
//    @GetMapping("/gifts/category-{category}")
//    public String allGiftsByCategory(@PathVariable String category, Model model) {
//        List<GiftViewModel> allGiftsByCategory = this.giftService.getAllGiftsByCategory(category);
//
//        model.addAttribute("allGiftsByCategory", allGiftsByCategory);
//        CategoryNameEnum[] allCategories = CategoryNameEnum.values();
